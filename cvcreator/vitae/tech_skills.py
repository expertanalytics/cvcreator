...
def get_skills(skill_categories: List[SkillCategory]) -> List[TechnicalSkill]:
    ...
    for skcat in skill_categories:
        label = skcat.category
        skills = skcat.technical_skills
        # Validation of skill categories and skills
        assert label in skills_data["allowed_labels"], ValueError(f"{label}: Invalid skill label")
        _validate_skills(skills, skills_data)
